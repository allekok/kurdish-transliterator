...
function ar2IL(s) {
	...
	return add_bizroke(replace_sure(replace_notsure(
		replace_sure(standardizing(s), before),
		notsure, determine_notsure), after),
			   n, v, bizroke)
}
function ar2lat(s) {
	const sure = [["أ","ئە"],
...
function ar2per(s) {
		...
		for(let i = 0; i < str.length-2; i++) {
			if(str[i] == str[i+1] && is_(str[i], n) &&
			   is_(str[i-1], v) && is_(str[i+2], v))
				str = str_replace_pos(str[i]+str[i],
						      str[i]+tashdid,
						      str,
						      i)
		}
	...
	function determine_hemze(s) {
		if(s.startsWith("ئ"))
			return determine_hemze(str_replace_pos(
				"ئ", "ا", s, 0))
...
function apply_to_words(str, fun) {
	...
	while(str[i] !== undefined) {
		...
		while(str[i] !== undefined &&
			...
			token += str[i++]
		if(!token)
			while(str[i] !== undefined && include.indexOf(
				str[i].toUpperCase()) !== -1)
				token += str[i++]
		new_str += fun(token)
	}
